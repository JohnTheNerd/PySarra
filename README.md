...
A Python application that can read from Sarracenia and relay the messages however you'd like. It is designed to be significantly simpler than the official client, and is meant for people who don't need all the bells and whistles. It is only tested on Linux and is confirmed not to work on macOS due to strange TLS issues I never got to resolve.

**Note:** Sarracenia currently returns a TLS certificate with the CommonName attribute of `weather.gc.ca` while the hostname is `dd.weather.gc.ca`, breaking TLS validation. As a workaround, I have disabled TLS hostname validation as I do not know of a way to make the Python SSL module validate against another hostname than the one provided in the certificate. This workaround makes it possible for an attacker to run a [man-in-the-middle attack](https://en.wikipedia.org/wiki/Man-in-the-middle_attack) against PySarra, so you should only run it on networks you trust!
